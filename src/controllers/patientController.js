...
const updatePatientInfo = async (req, res) => {
  try {
    ...
    const tx = await adminContract.updatePatientInfo(patientId, name, dateOfBirth, phoneNumber, emergencyContact)
...
const uploadSelfRecord = async (req, res) => {
  try {
    ...
    // Upload self-record
    const tx = await patientContract.uploadSelfRecord(cid, fileName, recordType, description, { from: patientAddress })
