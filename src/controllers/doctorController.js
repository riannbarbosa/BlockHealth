...
/**
 * Get doctor patients (for doctors to view)
 */
const getPatients = async (req, res) => {
  try {
    const { doctorId } = req.query

    if (!doctorId || !ethers.isAddress(doctorId)) {
      return res.status(400).json({
        success: false,
        message: 'doctorId inválido ou ausente',
      })
    }
    ...
    const patientAddresses = await medicContract.getDoctorPatients(doctorId)

    const patients = await Promise.all(patientAddresses.map(addr => adminContract.getPatientInfo(addr)))
    ...
    res.status(200).json({
      ...
      data: patients.map(p => ({
        ...
        isActive: p.isActive,
        registrationDate: new Date(Number(p.registrationDate) * 1000).toISOString(),
      })),
    })
  } catch (error) {
...
const addPatientRecord = async (req, res) => {
  try {
    ...
    const tx = await medicContract.addMedicalRecordByAdmin(cid, fileName, patientId, diagnosis, treatment, doctorId)
...
const deactivateRecord = async (req, res) => {
  try {
    ...
    const tx = await medicContract.deactivateRecordByAdmin(patientId, recordIndex, doctorId)
    const receipt = await tx.wait()
