...
const setupContract = async app => {
  try {
    ...
    // Load AdminContract
    try {
      const adminContractData = JSON.parse(fs.readFileSync(path.join(contractsPath, 'AdminContract.json'), 'utf8'))
    ...
    // Load MedicContract
    try {
      const medicContractData = JSON.parse(fs.readFileSync(path.join(contractsPath, 'MedicContract.json'), 'utf8'))
    ...
    try {
      const patientContractData = JSON.parse(fs.readFileSync(path.join(contractsPath, 'PatientContract.json'), 'utf8'))
